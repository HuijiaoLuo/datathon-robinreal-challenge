...
def build_search_tool_result(
    *,
    ...
    # build a human-readable summary of the listings
    if count == 0:
        text = f"No listings found for '{query}'"
    else:
        lines = [f"🏠 Found {count} listings for '{query}':\n"]
        for i, item in enumerate(listings, 1):
            l = item["listing"]
            lines.append(
                f"{i}. {l.get('title', 'N/A')}\n"
                f"   📍 {l.get('city', 'N/A')} | 🛏️ {l.get('rooms', 'N/A')} rooms | 💰 {l.get('price_chf', 'N/A')} CHF\n"
                f"   🔗 {l.get('original_listing_url', 'N/A')}\n"
            )
        text = "\n".join(lines)
