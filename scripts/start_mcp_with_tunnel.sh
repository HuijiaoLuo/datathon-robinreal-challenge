#!/bin/sh
echo "Waiting for tunnel URL..."
# Wait for the tunnel file to appear, up to 30 seconds
TIMEOUT=30
COUNTER=0
while [ ! -f /tmp/tunnel_url.txt ] || [ ! -s /tmp/tunnel_url.txt ]; do
    echo "Waiting for cloudflared to generate tunnel URL... (${COUNTER}/${TIMEOUT})"
    sleep 2
    COUNTER=$((COUNTER + 1))
    if [ $COUNTER -ge $TIMEOUT ]; then
        echo "Timeout, using localhost as public address"
        TUNNEL_URL="http://localhost:8001"
        break
    fi
done

if [ -f /tmp/tunnel_url.txt ] && [ -s /tmp/tunnel_url.txt ]; then
    TUNNEL_URL=$(cat /tmp/tunnel_url.txt)
fi

echo "Found tunnel URL: $TUNNEL_URL"

export APPS_SDK_PUBLIC_BASE_URL=$TUNNEL_URL
export APPS_SDK_LISTINGS_API_BASE_URL=http://api:8000
export APPS_SDK_PORT=8001

echo "Starting MCP server"
echo "Public URL: $APPS_SDK_PUBLIC_BASE_URL"
echo "API URL: $APPS_SDK_LISTINGS_API_BASE_URL"

exec uvicorn apps_sdk.server.main:app --host 0.0.0.0 --port 8001